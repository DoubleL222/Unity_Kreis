...
public class GameManagerMulti : MonoBehaviour {
  ...
  PowerUpManager PUM;
	...
	public void SpawnRings(float[,] RingSizes){
		rings = new List<RingManagerMulti> ();
		for (int i = 0; i < RingSizes.GetLength (0); i++) {
			rings.Add (new RingManagerMulti (RingSizes[i,0], RingSizes[i,1]));
		}
	}
...
//    PUM.spawnPowerups = true;
	...
	public void FinalDestruction(float delayStep){
		float mem = delayStep;
		foreach (RingManagerMulti rm in rings) {
