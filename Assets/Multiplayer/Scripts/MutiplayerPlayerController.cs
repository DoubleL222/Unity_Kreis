...
public class MutiplayerPlayerController : PolarPhysicsObject {
	public MultiplayerControlls MultiControlls;
	...
	void FixedUpdate () {
		...
		StartUpdate ();
		List<ControllCommands> CC = MultiControlls.getPlayerInput (keys);
		if (CC.Count > 0) {
			foreach (ControllCommands command in CC) {
				if (command == ControllCommands.left) {
				...
				} else if (command == ControllCommands.right) {
					if (!boosterEmiter.activeSelf) {
						boosterEmiter.SetActive (true);
					}
					Vector2 f = new Vector2 (movementForce, 0);
				...
				if (command == ControllCommands.gravity && isGrounded && (lastGravityChange + gravityChangeRate) < Time.fixedTime) {
					lastGravityChange = Time.fixedTime;
					gravity = -gravity;
					SoundM.PlayJumpClip ();
				}
				if (command == ControllCommands.shoot && isGrounded && (lastShoot + fireRate) < Time.fixedTime) {
